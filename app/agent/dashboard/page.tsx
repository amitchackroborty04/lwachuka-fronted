        ...
        <div className="min-h-screen bg-[#F8F9FA]">
            ...
            <div className="p-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {stats.map((card) => (
                        <Card key={card.title} className="bg-white border-0 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] rounded-2xl overflow-hidden">
                            <CardContent className="p-7">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <p className="text-[14px] font-medium text-gray-400 capitalize">{card.title}</p>
                                        <p className="text-4xl font-bold text-[#0D1B2A] mt-2 mb-1">
                                            {isLoading ? "..." : card.value}
                                        </p>
                                        <p className="text-[13px] text-gray-400 font-normal">{card.subtitle}</p>
                                    </div>
                ...
                <Card className="bg-white border-0 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] rounded-2xl p-8">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-lg font-bold text-[#0D1B2A]">Total Listings</h2>
                            <p className="text-sm text-gray-400">Monthly overview of your growth</p>
                        </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={dummyChartData}>
                                ...
                                <Area
                                    ...
                                    fillOpacity={1}
                                    fill="url(#colorValue)"
                                    dot={{ r: 4, fill: '#0D1B2A', strokeWidth: 2, stroke: '#fff' }}
                ...
                {/* Quick Actions */}
                <div className="pb-10">
                    <h2 className="text-lg font-bold text-[#0D1B2A] mb-6">Quick Actions</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <Link key={action.label} href={action.href}>
                                <div className="group flex items-center justify-between p-6 bg-white border border-gray-100 rounded-2xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.03)] hover:shadow-md hover:border-[#0D1B2A]/10 transition-all cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        <div className={cn("p-3 rounded-xl transition-colors", action.bgColor)}>
                                            <action.icon className="h-6 w-6 text-[#0D1B2A]" />
                                        </div>
                                        <span className="text-[16px] font-bold text-[#0D1B2A]">{action.label}</span>
                                    </div>
